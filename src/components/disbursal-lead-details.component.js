...
import { getLeadProfile, getAddressProofDoc, getDocByType, loanUpdate, getGroupList, getBankDocuemnt, groupEmailSearch, clearEmailSearch, getAddress, getLeadOtherDocuemnt, getLeadPanDocuemnt, getLeadProfileDocuemnt, getLenderLeadDetails, openLeadProfileModel, getRepayment, geBbureau, getDocumentCount, sendEmailCAM } from "../actions/users";
import {
  CircularProgressbar,
...
class LeadDetails extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      modalTitle: '',
      book_model: false,
      book_loan_model: false,
      drop_case_model: false,
      loan_booked: false,
      net_tenure: '',
      loan_amount: '',
      upfront_amount: '',
      ...
      amount: null,
      payment_date: null,
      current_status: 'Ready to disburse',
      bank_data: [],
      disUploadFile: null,
      harddata: {},
      bureaurs: [],
    ...
    this.props.dispatch(getLenderLeadDetails(opp_data)).then((response) => {
      if (response.status === "success") {
        ...
        this.setState({
          ...
          opp_sfid: getData && getData.opp_sfid ? getData.opp_sfid : null,
  ...
  handleBookModel = (value) => {
    this.setState({ book_loan_model: value })
  }

  handleBookLoanModel = (value) => {
    this.setState({ book_model: value, book_loan_model: false })
  }

  handleDropCase = (value) => {
    this.setState({ drop_case_model: value })
  }

  handleDrop = () => {
    let sfid = localStorage.getItem('user_sfid');
    let data = {
      oppertunity_sfid: this.state.opp_sfid,
      stage_name: 'Lead Dropped',
      loggedin_user: sfid
    }
    this.props.dispatch(loanUpdate(data)).then((response) => {
      if (response && response.status == "success") {
        this.setState({ drop_case_model: false });
  ...
  handleBook = () => {
    ...
    let data = {
      oppertunity_sfid: this.state.opp_sfid,
      stage_name: 'Ready to disburse',
      loggedin_user: sfid,
      disbursal_amt: parseInt(this.state.amount),
      utr_num: this.state.utr_no,
      disbursal_date: this.state.payment_date,
    }
    this.props.dispatch(loanUpdate(data)).then((response) => {
      if (response && response.status == "success") {
        this.setState({ loan_booked: true, book_model: false });
        setTimeout(() => {
          this.setState({ loan_booked: false });
          this.props.history.goBack();
        }, 3000);
      }
    });
  ...
  render() {
    ...
    const { utr_no, amount, payment_date, book_loan_model, drop_case_model, book_model, loan_booked, bank_data, reason, harddata, bureaurs, datarrr, bureaurData, currentAddress } = this.state
    if (!user_id) {
    ...
    return (
      <>
            ...
            <div className="container-fluid lead_details_header">
              <div className="row align-items-center">
                <div className="d-flex">
                  <div style={{ flex: '1' }}>
                    <div className="backicon float-left"><a href="/disbursalPending"> <img src="img/icon_back.svg" /></a></div>
                    <div className="d-flex">
                      ...
                      <div className="pr_texts">
                        <h5 className="d-block">{this.state.sfid}<span><b>Disbursal Pending</b></span></h5>
                ...
                <div className="col-md-12">
                  <div className="row mt-3">
                    ...
                    <div className="col-mt-8 text-right tbuttons" style={{ marginLeft: '10rem' }}>
                      <button className="btn btn-primary csy1 mr-4"><img src="img/icon_tick.svg" /><b> Approved by: {this.state.approvedBy}<span className="d-block">On {this.state.approveDate}</span></b>
                      </button>
                      <button type="button" onClick={() => this.handleBookModel(true)} className="btn btn-dark mr-4"><img src="img/icon_book_loan.svg" /> Book Loan</button>
                      <button className="btn btn-primary" type="button" onClick={() => this.handleDropCase(true)}><img src="img/icon_drop_case.svg" /> Drop Case</button>
                    </div>
                  </div>
                                  ...
                                  <div className="tab-content">
                                    ...
                                    <div className="tab-pane" id="fstatements" role="tabpanel">
                                      ...
                                      <div className="row">
                                        ...
                                        <div className="col-md-12 isdownload">
                                          <h6><b>Salary Slips</b> <img src="images/download.svg" /><nav className="float-right">
        ...
        <Modal show={book_loan_model} className="right-modal-width">
          <div className="modal-body text-center">
            <div className="d-inline-block">
              ...
              <button type="button" onClick={() => this.handleBookLoanModel(true)} className="btn-black w-100 mb-4"><h4>Book Case</h4></button>
              <button type="button" onClick={() => this.handleBookModel(false)} className="later">Later</button>
            </div>
          </div>
        </Modal>
        {/* Approved */}
        ...
        {/* Book Modal */}
        <Modal show={book_model} className="right-modal-width">
          <div className="modal-body text-center">
            <div className="row">
              <h4 className="mb-4">Disbursement Details</h4>
              <button type="button" onClick={() => this.handleBookLoanModel(false)} className="close ml-auto mb-5" >
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
            <div>
              <div className="form-group">
                <input
                  type="text"
                  className="form-control disPendIn"
                  id="utr_no"
                  placeholder="UTR No."
                  name="utr_no"
              ...
              <div className="row mt-4 mb-4">
                <div className="col-md-5">
                  <input type="file" id="actual-btn" hidden accept="application/pdf,image/jpeg" onChange={(e) => { this.uploadFile(e) }} />
                  <label for="actual-btn" className="upload-btn">{this.state.disUploadFile ? "Choose File" : "Upload File"}</label>
                </div>
                <div className="col-md-7">
                  {
                    this.state.disUploadFile ?
                      <>
                        <p class="mt-4" style={{ wordWrap: "break-word" }}>{this.state.disUploadFile.name}</p>
                      </>
                      :
                      <>
                        <p className="up-text1">Upload the disbursed here</p>
                        <p className="up-text2">File Supported: PDF, JPEG</p>
                      </>
                  }
                </div>
              </div>
              <button
                ...
                disabled={utr_no && amount && payment_date ? false : true}
                className={utr_no && amount && payment_date ? "disConBtn" : "disConDisable"}
              >Confirm</button>
            </div>
          </div>
        </Modal>
        ...
        {/* Loan Booked */}
        <Modal show={loan_booked} className="right-modal-width">
          <div className="modal-body text-center">
            <div className="d-inline-block">
              <div className="d-inline-block">
        ...
        {/* Dropped */}
        <Modal show={drop_case_model} className="right-modal-width">
          <div className="modal-body text-center">
            <div className="d-inline-block">
              <div className="d-inline-block">
              ...
              <div>
                <textarea name="reason" onChange={this.handleChange} className="textarea2" placeholder="Mention the reason here *" />
              </div>
              <div className="pt-5">
                <button type="button"
                  onClick={this.handleDrop}
                  disabled={reason ? false : true}
                  className={`${reason ? 'btn-black' : ''} w-100 mb-4`}><h4>Drop Case</h4></button>
                <button type="button" onClick={() => this.handleDropCase(false)} className="later">Later</button>
              </div>
            </div>
