...
class AllLeads extends Component {
  ...
  componentDidMount() {
    ...
    this.props.dispatch(getLeads(leadObj, pagination_obj,dateRangeObj,this.state.selectedCase)).then((response) => {
      if (response && response.status && response.status === "success") {
        ...
        console.log("contDet", contDet);
        const dropped = contDet && contDet.dropped ? contDet.dropped : null;
        this.setState({ dropped: dropped, totalDropped: dropped.total });
      }
    });
  ...
  openLeads = (id) => {
    ...
    history.push('/dropped_lead_details');
  }
...
changeSelectedStatus = (e) => {
  ...
  render() {
    ...
    const { selectedTab, dropped, perPage, totalDropped } = this.state;
    if (!user_id) {
      return <Redirect to="/login" />
    }
    let tabledata = searchActive ? searchArr : leads;
    const totalPages = Math.ceil(totalDropped / perPage);
    return (
      <>
        <Helmet>
          <title>Dropped</title>
            ...
            <div className="col-md-12">
              <ul className="leads_boxinfo">
                <li className="col_1">
                  ...
                  <span className="atsize">Dropped Cases</span>
                </li>
              </ul>
            </div>
          </div>
                    ...
                    <ul className="leads_boxinfo">
                      <li className="col_1">
                        <div className="inner  d-flex">
                          <div className="w-50">
                            <img src="images/arrow-from-drop.svg" />
                            <span className="atsize">Dropped Cases</span>
                          </div>
                          <div className="w-50">
                            <div className="d-flex align-items-center justify-content-end">
                              <i className="fa fa-inr d-inline-block mr-1" aria-hidden="true"></i>
                              <span className="title d-inline-block mr-1">{dropped && dropped.amount ? dropped.amount : 0}</span>
                              {/* <span className="title">Cr</span> */}
                            </div>
                            <div className="text-right"><span className="title d-inline-block mr-1">{dropped && dropped.total ? dropped.total : 0}</span></div>
                          </div>
                        </div>
                      </li>
                    ...
                    <div className="table-responsive dark_header">
                      ...
                      <div >
                        <div className="d-flex justify-content-end pb-4">
                          <div className="p-3">
                            {this.state.currentPage}-{totalPages} of {this.state.totalDropped}
                          </div>
                          <div className="p-3">
                            Cases/page
                          ...
                          <div className="p-2">
                            ...
                            <button className="btn bg-light ml-2 shadow-lg" type="button" onClick={() => this.handleNextPrevPage("next")} disabled={this.state.perPage > tabledata.length || this.state.perPage == this.state.totalDropped ? true : false}> Next Page <i className="fa fa-long-arrow-right"></i></button>
                          </div>
...
function mapStateToProps(state) {
  const { isLoading, user_sfid, isLoggedIn, user_id, token_id } = state.auth;
