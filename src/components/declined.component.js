...
class AllLeads extends Component {
  ...
  componentDidMount() {
    ...
    this.props.dispatch(getLeads(leadObj, pagination_obj,dateRangeObj,this.state.selectedCase)).then((response) => {
      if (response && response.status && response.status === "success") {
        ...
        const declined = contDet.declined ? contDet.declined : null;
        this.setState({ declined: declined, totalDeclined: declined.total });
      }
    });
  ...
  openLeads = (id) => {
    ...
    history.push('/declined_lead_details');
  }

  generatePDF = () => {
    var doc = new jsPDF('p', 'pt');
    doc.text(20, 20, 'Declined Leads')
    doc.addFont('helvetica', 'normal')
    doc.autoTable({ html: '#dataTables' })

    doc.save('declined.pdf')
  }
...
changeSelectedStatus = (e) => {
  ...
  render() {
    ...
    const { selectedTab, declined, perPage, totalDeclined } = this.state;
    if (!user_id) {
      return <Redirect to="/login" />
    }
    let tabledata = searchActive ? searchArr : leads;
    const totalPages = Math.ceil(totalDeclined / perPage);
    return (
      <>
        <Helmet>
          ...
          <div className="row align-items-center">
            <div className="col-md-12">
              <ul className="leads_boxinfo">
                <li className="col_1">
                  <span className="title"><img src="images/clo_loans.svg" /> 20</span>
                  <span className="atsize">Declined Cases</span>
                </li>
              </ul>
            </div>
          </div>
                        ...
                        <div className="inner  d-flex">
                          ...
                          <div className="w-50">
                            <div className="d-flex align-items-center justify-content-end">
                              ...
                              <span className="title d-inline-block mr-1">{declined && declined.amount ? declined.amount : 0}</span>
                              {/* <span className="title">Cr</span> */}
                            </div>
                            <div className="text-right"><span className="title d-inline-block mr-1">{declined && declined.total ? declined.total : 0}</span></div>
                          </div>
                        </div>
                      </li>
                    </ul>
              ...
              <a href={void (0)} ><button onClick={this.openDownload} className="icon-button border-0"><img src="images/icons/download.svg" /> Download All Cases</button></a>
                ...
                <div className="trw">
                  ...
                  <a href={void (0)} className="cursor-point" onClick={this.openEmailReport}>
                    <button className="icon-button mr-2"><img src="images/icons/email.svg" /> Email Report</button></a>
                    ...
                    <div className="table-responsive dark_header">
                          ...
                          <tr>
                            <th>
                              <div className="single_check">
                                <input type="checkbox" id="all_check" onClick={(e) => this.changeSelectedStatus(e)}/>
                                <label>#</label>
                              </div>
                            </th>
                            ...
                            (
                              tabledata.map((item, index) => (
                                <tr className="shown" key={index}>
                                  ...
                                  <td>
                                    <b className="underline d-block cursor-point cursor-point"><a href={void (0)} onClick={() => this.openLeads(item.opp_id)}>{item.opp_id}</a></b>
                      ...
                      <div >
                        <div className="d-flex justify-content-end pb-4">
                          <div className="p-3">
                            {this.state.currentPage}-{totalPages} of {this.state.totalDeclined}
                          </div>
                          <div className="p-3">
                            Cases/page
                          ...
                          <div className="p-2">
                            ...
                            <button className="btn bg-light ml-2 shadow-lg" type="button" onClick={() => this.handleNextPrevPage("next")} disabled={this.state.perPage > tabledata.length || this.state.perPage == totalDeclined ? true : false}> Next Page <i className="fa fa-long-arrow-right"></i></button>
                          </div>
...
function mapStateToProps(state) {
  ...
  const { leads, leadsCount, group_recipient,searchArr,group_list } = state.user;
  const { message } = state.message;
  const { searchActive,open_email_report, open_download_report, closeDownloadReport } = state.model;
